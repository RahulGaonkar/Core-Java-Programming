 ...
 * User: blangel
 *
 * Fix the inner class leak. When invoked before changing, this will either run
 * out of memory (JVM will end with an java.lang.OutOfMemoryError) or it'll
 * print a statement saying too much memory was used after 30 seconds.
 *
 */
public class Faucet {
    ...
    private static class Water {
        ...
        private Water(Random random, int flowLength) {
    ...
    public static void main(String[] args) {
        ...
        FancyOutput.print("Starting with ^yellow^%s^r^ MB used memory (running faucet for ~30 seconds)%n",
                (usedMemory / BYTE_TO_MB));
        ...
        Runtime.getRuntime().gc(); // students, do not do this in practice (doing simply for ease of demonstration
                                   // on leak in homework)
    ...
    public Water turnOn() {
        return new Water(random, flow.length);
    }
